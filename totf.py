...
def _get_filenames_and_classes(dataset_root):
  with tf.python_io.TFRecordWriter('output.tfrecords') as record_writer:
    for dirname in os.listdir(dataset_root):
      dirpath = os.path.join(dataset_root, dirname)  # zitie/yzq
      print(dirpath)
      if os.path.isdir(dirpath):
        ...
        for filename in os.listdir(dirpath):
          ...
          with tf.Session() as sess:
            ...
            img_data = tf.image.resize_images(img_data, [128, 128])
...
def main():
    _get_filenames_and_classes('zitie')
